...
**Наблюдения:**
- На хорошо текстурированных объектах трекинг устойчивый: большинство точек корректно отслеживаются на протяжении всего ролика.
- При умеренном движении оптический поток Лукаса–Канаде сохраняет плавность и не даёт резких скачков позиционирования.
- На видео с большим количеством фона (когда объект занимает малую часть кадра) система может ошибочно включать фоновые точки в расчёт, что смещает bbox.

### 3.1. Видео собственного набора
Были записаны видео с разными объектами.
**Наблюдения:**
- При плавных перемещениях bounding box следует за объектом.
- При частичной потере точек (например, из-за перспективы или бликов) bounding box может менять свой размер, иногда охватывает не весь объект.
- Столкновение с полностью однородным участком объекта приводит к частичной потере точек.



## 4. Выводы по работе 

В ходе лабораторной работы был реализован и протестирован простой, но классический алгоритм трекинга на основе ключевых точек.

- **Практическое понимание основных ограничений** feature-based tracking:
  - зависимость от текстуры,
  - чувствительность к изменениям яркости,
  - невозможность восстановления потерянных точек без дополнительной логики,
  - ограниченность ось-выровненного bounding box.

Несмотря на простоту, данный подход демонстрирует фундаментальные идеи, лежащие в основе современных систем трекинга: использование ключевых точек, локальных оптических смещений, пирамидальных представлений изображения и фильтрации надёжных соответствий.
